...
export type AddressFormValue = Pick<AddressFragment, Exclude<keyof AddressFragment, 'country'>> & {
  countryCode: string;
};
...
export class CheckoutShippingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.shippingAddress$ = shippingData$.pipe(map((data) => data.activeOrder && data.activeOrder.shippingAddress));
  ...
  setShippingAddress(value: AddressFormValue | AddressFragment) {
    ...
    this.dataService
      .mutate<SetShippingAddressMutation, SetShippingAddressMutationVariables>(SET_SHIPPING_ADDRESS, {
        input,
      })
      .subscribe(() => {
    ...
    if (shippingMethodId) {
      this.stateService
        ...
        .pipe(
          ...
          mergeMap(() =>
            this.dataService.mutate<SetShippingMethodMutation, SetShippingMethodMutationVariables>(
              SET_SHIPPING_METHOD,
              {
                id: shippingMethodId,
              }
  ...
  private setCustomerForOrder() {
    if (this.contactForm.valid) {
      return this.dataService
        .mutate<SetCustomerForOrderMutation, SetCustomerForOrderMutationVariables>(SET_CUSTOMER_FOR_ORDER, {
